...
## [2026-04-28] PM · Audit + 拆解规划 · 待 Founder 拍板 spawn

Founder 反馈:Stage 1 三档"鸡肋",数据没问题,可以下架 + 进 Stage 2。

PM 已完成全套文档 audit + 补齐:
- 新建 `daily-sync/2026-04-27.md`(三轮 hotfix 完整记录)
- 新建 `daily-sync/2026-04-28.md`(Stage 1 verdict + Stage 2 启动)
- DECISIONS 追加 D017(下架)+ D018(Stage 2 启 C 方案)
- KNOWN_ISSUES 追加 GRAY-007(scroll-view enable-flex bug)+ STAGE-1 verdict
- TODAY_FOCUS / PROJECT_STATUS / PENDING 全套刷新

待 Founder 对下面拆解规划说"可以"再 spawn:
- @backend / @frontend / @tester 三档下架(估 1-2 天)
- @backend / @frontend Stage 2 C 方案上下文注入(估 3-5 天)


---

## [2026-04-28] PM · 三档下架 + Stage 2 C 方案 spawn(D018a 细节锁)

Founder 拍板细节:按钮 「✨ 基于此继续优化」/ 上限 3 轮 / system prompt 用 PM 草稿。

策略:**1 轮 spawn 3 teammate 并行**,每人做 Phase 1(三档下架自己负责的部分)+ Phase 2(C 方案 上下文注入自己负责的部分)合并任务,避免两轮 spawn 协调成本。

- @backend: stream.py + stream_en.py 删 directive(31 端点)+ 加 context_prompt 解析支持(同时改省一次)
- @frontend: index.* 删 complexity UI + 加「✨ 基于此继续优化」按钮 + 3 轮 counter + 视觉
- @tester: 删 test_complexity.py + 加 test_context_injection.py 至少 3 stub + 全量回归


---

## [2026-04-28 14:31] @frontend · Phase 1 三档下架 + Phase 2 D018a 上下文注入(合并)完成
...
`pages/index/{wxml,wxss,js}` 三档相关实施全部清除:
- WXML 删 `.complexity-selector` 整段 + 2 处 class binding(.input-area / .result-card)+ `.professional-badge`
- WXSS 删整片 Stage 1 三档样式区(L1138-1257,9 selector + 3 档 .active 变体 + .input-area-professional / .result-card-professional / .professional-badge)
- JS 删 data 块 `currentComplexity` + `complexityOptions` + method `switchComplexity` + `generateContent` body 的 `complexity` 字段 + `generateImageDescription` URL 的 `&complexity=...`
- **保留**:Hero 文案"专业创作者的 AI Prompt 工作台"(D017 明示)
...
### Phase 2 D018a Stage 2 C 方案 上下文注入 ✅

按 D018a 产品契约(字段名 `context_prompt` / 3 轮上限前端硬约束 / 「✨ 基于此继续优化」按钮)实施:

**JS 改动**(pages/index/index.js):
...
- 双路径挂 context_prompt:
  - `generateContent` body(L2378-2381,spread 写法)
  - `generateImageDescription` URL query(L487-490,&context_prompt=)
  - 仅在 `refinementRound > 0 && previousOutput` 时挂(fallback 友好,后端兼容)
- 双 reset 点:`onInputChange` + `onReferenceInputChange`,改输入文本视为新主题 → reset

**WXML 改动**(pages/index/index.wxml):
- 加「第 N 轮迭代」徽标(result-card 顶部左上角,wx:if refinementRound > 0)
- 加「✨ 基于此继续优化 (剩 N 次)」按钮(result-card 内,result-content 之后)
- 加上限态灰提示(refinement-done)
- 增加 `!isGenerating` 守卫,生成中按钮和上限态都不显示

**WXSS 改动**(pages/index/index.wxss):
- 7 个新 selector:`.refinement-area` / `.refine-btn`(渐变绿→蓝 pill)/ `.refine-btn::after` / `.refine-btn-hover` / `.refine-counter` / `.refinement-done` / `.refine-done-text` / `.refinement-badge`(占据原 .professional-badge 视觉位置 + 改用左上角 + 蓝色辅色 #3F88C5)
...
| grep refinementRound / context_prompt 在 pages/index/* | 多处命中 ✅ |
| 主包尺寸净变化 | **+9 B**(wxml -87 + wxss -1214 + js +1310,远低于 5KB 上限)|
| 新增 px 单位 | 0(全 rpx)|
| 微信合规 | rpx / WXML / setData / 无 DOM / 无 npm 全绿 |
| scope | 只动 pages/index/{wxml,wxss,js},未碰 sumai/ 未碰 tests/ 未碰 app.js |

### 给 @backend 的对齐提示

- 前端已按契约挂 context_prompt 字段(双路径)
- **建议**:`generateImageDescription` 走 URL query 携带 context_prompt,长 prompt 可能超 nginx 8KB header buffer。Round 1 联调时若测出 414 → 双方协调改 POST / 调 nginx / 前端做长度截断
- complexity 字段已删,@backend 同步删 stream.py / stream_en.py 的 directive 即可

### 给 @tester 的回归点

详见 `.claude/agents/frontend-progress/context-for-others.md` 的"给 @tester 的上下文"章节,7 项手动回归点 + 3 项建议自动测试 stub。

### 文件变更清单(给 PM 统一 commit)
...
## [2026-04-28] @tester · Phase 1 + Phase 2 完成(三档下架测试清理 + Stage 2 上下文注入 sensor)

任务来源: PM(Coordinator Opus 4.7)D017 + D018a 合并 spawn,与 @backend / @frontend 并行。

### 一、Phase 1 — 三档相关测试清理(D017)✅

1. **删除 `sumai/tests/test_complexity.py`**(R3-D 创建的 3 个 active 测试)
   - 删前状态:基线跑出 3 failed(@backend 已删 stream.py 顶部 COMPLEXITY_DIRECTIVES dict + resolve_complexity 函数,导致 sensor 凋零)
...
2. **残留扫描**:
   ...
   - `grep -rni "complexity"` 仅命中两处:
     - (a) `sumai/tests/README.md` 旧表格条目,已清理(改为列出 D017 后已下架的 3 文件 + Stage 2 新增 test_context_injection.py)
     - (b) `sumai/tests/test_sse_is_pro_branch.py:113` `def test_is_pro_determines_system_prompt_complexity` — 此处"complexity"指 prompt 的精细度(由 max_tokens 控制),与 D016/D017 三档无关,**保留**

### 二、Phase 2 — Stage 2 上下文注入测试 stub(D018a)✅
...
| 3 | `test_resolve_context_truncates_oversized_input` | active 功能(隔离 exec 加载) | ✅ PASS |
| 4 | `test_endpoints_inject_context_when_present` | skip stub(等 Flask client + LLM mock 真实化) | ⏸️ SKIP |

**关键技术决策(Test 3)**:
- 不 import 整个 stream.py(顶部 `from anthropic import Anthropic` + `os.environ['MYSQL_PASSWORD']` 在本地无生产凭证时会崩)
- 改用 regex 抽取 `def resolve_context(data):` 函数体源码 + `exec()` 到隔离 namespace,只跑纯函数逻辑
- 验证四种场景:None / 空串 / 6000 字符截断(≤5050 上限)/ 边界 5000 字符不截断

**断言覆盖**:
- 模板常量 `CONTEXT_INJECTION_TEMPLATE` (zh) + `CONTEXT_INJECTION_TEMPLATE_EN` (en) 模块级赋值存在
- zh 模板含【上下文】+ 继续优化关键短语
- en 模板含 [Context] + refine(case-insensitive)关键短语
- 两文件模板均含 `{previous_output}` 占位符(.format() 必备)
- `def resolve_context(data):` 签名存在
- 函数体 `data.get('context_prompt'...)` 字段读取存在
- 5000 字符截断保护存在(防御 max_tokens 被吃光)

### 三、全量回归 ✅

| 测试套 | 任务前基线 | 删 test_complexity 后 | + test_context_injection 后 | 任务要求最低 | 实际 |
|---|---|---|---|---|---|
| xuhua-wx | 18 passed | 18 | **18 passed** | 18 | ✅ 持平 |
| sumai passed | 92 (含 3 fail) | 89 | **92 passed** | ≥89 | ✅ 持平基线 |
| sumai skipped | 95 | 95 | **96 skipped** | (95-3+1=93) | ✅ +1(新 stub) |
| sumai xfailed | 3 | 3 | **3 xfailed** | 3 | ✅ |
| sumai xpassed | 2 | 2 | **2 xpassed** | 2 | ✅ |
| sumai total | 195(含 3 fail) | 189 | **193** | — | -2 净变化(-3 + 4) |

**说明**:任务要求"基线 92 passed"指的是 R3-D 收尾的 92,但本轮启动时实测 sumai 已经是 89 passed + 3 failed(@backend 已先删 COMPLEXITY_DIRECTIVES dict 致 test_complexity 凋零)。删除该文件后 89 passed 干净基线,加入新 sensor 后回到 92 passed,**与 R3-D 收尾基线对齐,零回归**。

### 四、HARNESS_HEALTH.md 更新 ✅

- Sensor 表格删除 `complexity 三档 directive (D016)` 行(D017 下架)
- 新增 Stage 2 D018a 行:`test_context_injection.py(3 active + 1 skip)` 状态 PASS/SKIP
- 最近变更记录追加本次 D017 清理 + D018a 新 sensor 条目

### 五、给 @backend 的发现

**正面**:你已实施 `CONTEXT_INJECTION_TEMPLATE` + `resolve_context()` 在 stream.py 和 stream_en.py 顶部,且每个 SSE 端点 generate() 内已加 `ctx = resolve_context(data); final_system = system + TEMPLATE.format(...) if ctx else system` 模式。grep 命中 stream.py 12+ 处 + stream_en.py 13+ 处,覆盖完整 ✅

**Follow-up 建议**(P2 非紧急):
1. Test 4 端点级集成测试激活前需 Flask test client + LLM mock 真实化(已写完整断言代码作 TODO)
2. 是否考虑加 sensor `test_all_sse_endpoints_call_resolve_context` — 静态扫所有 `def botPromptStream / aiAgentStream / ... ` 函数体内是否有 `ctx = resolve_context(data)` 行?可防御未来新增端点漏注入

### 六、给 @frontend 的协调

- 三档 UI 下架 + 「✨ 基于此继续优化」按钮上线后,前端的 `context_prompt` 透传字段名约定 = `context_prompt`(后端已用此字段名,详见 stream.py:40 `data.get('context_prompt', '')`)
- 3 轮 counter 由前端强制(后端不感知轮次,每次都是 stateless)
- 真机回归 5 关键流程建议加一条:"基于此继续优化"按钮点 1/2/3 次后第 4 次应被前端 disable

### 七、修改文件清单(等 PM 统一 commit)

**sumai 仓库**(独立 git remote):
- `sumai/tests/test_complexity.py`(已 git rm)
- `sumai/tests/test_context_injection.py`(新建,~250 行)
- `sumai/tests/README.md`(2 处更新:测试清单 + Stage 2 章节)

**xuhua-wx 仓库**:
- `.claude/agents/tester-progress/{current,completed,context-for-others}.md`(三件套全更新)
- `.team-brain/status/HARNESS_HEALTH.md`(Sensor 表 + 最近变更)
- `.team-brain/TEAM_CHAT.md`(本条追加)
...
## [2026-04-28 14:31] Backend · Stage 2 Phase 1 + Phase 2 合并 完成 ✅

按 PM spawn 指令,合并完成 Phase 1(D017 三档下架)+ Phase 2(D018a C 方案 上下文注入),只动 `sumai/stream.py` + `sumai/stream_en.py` 两文件。

### 一、Phase 1 · 三档下架(D017)

**stream.py 删除**:
- 模块级 `COMPLEXITY_DIRECTIVES = {...}` dict + 上方"Stage 1 / W2-4 / D016"注释
- `resolve_complexity(data)` 函数
- 17 处端点内 `system + COMPLEXITY_DIRECTIVES.get(resolve_complexity(data), "")` 引用

**stream_en.py 删除**:
- 同上(`COMPLEXITY_DIRECTIVES_EN` + `resolve_complexity` + 14 处端点引用)
...
### 二、Phase 2 · 上下文注入(D018a)

**stream.py 新增**(顶部):

```python
# Stage 2 / D018a: 上下文注入(Conversational Refinement)
# 用户点击"✨ 基于此继续优化"后,前端把上一轮 output 通过 context_prompt 字段传回,
# 后端把它包成上下文 block 追加到 system prompt 末尾...
# 上限 3 轮(D018a)由前端 counter 强制,后端不感知轮次。
...
**stream_en.py 新增**:`CONTEXT_INJECTION_TEMPLATE_EN`(英文翻译,截断标记 `...(truncated)`)+ `resolve_context()`(同结构)。

**31 端点统一注入模式**(stream.py 17 + stream_en.py 14):
...
| 文件 | 符号 | 计数 | 期望 | 状态 |
|---|---|---|---|---|
| stream.py | `CONTEXT_INJECTION_TEMPLATE` | 18 | ≥ 18(1 定义 + 17 端点) | ✅ |
| stream.py | `resolve_context` | 18 | 18 | ✅ |
| stream_en.py | `CONTEXT_INJECTION_TEMPLATE_EN` | 15 | ≥ 15(1 定义 + 14 端点) | ✅ |
| stream_en.py | `resolve_context` | 15 | 15 | ✅ |
...
- 基线说明: 任务交付时基线 92 passed;@tester 同轮已删 `test_complexity.py`(3 stub),所以 89 passed 是**预期值**。**0 failed,0 collection error,无回归**。
- @tester 新加的 `test_context_injection.py`(~250 行)我没跑(@tester 自己跑过),但若全 PASS 则总数会上升到约 100 passed。

### 五、关键设计决策

1. **后端不感知轮次** — 只看 `context_prompt` 字段是否存在 + 非空,不维护 round counter。轮次上限 3 由前端 counter 强制。未来若改上限,只动前端,后端零工作量。
2. **Fallback 友好** — 不传 / 传空 / 传 None / 全空白 → `resolve_context()` 返 None → 端点行为完全等同改动前(向后兼容,新前端 / 旧前端都能用)。
3. **5000 字符防御截断** — 防止上一轮巨大 output 吃光 max_tokens。截断时尾部加 `...(已截断)` / `...(truncated)`。
4. **不动任何 system prompt 字符串本身** — 只在末尾追加上下文 block,Stage 1 删除不影响 prompt 质量。
5. **stream_en.py 用独立英文模板** — `[Context] The prompt the user obtained from the previous round is: ...`,保持英文 LLM 输出语言一致性。
6. **不引入新 Python 包** — 只用 stdlib + 已有 anthropic/openai。

### 六、唯一特殊端点

`/describeImageStream`(stream.py L1736)的 ctx 注入是 **4 空格缩进**(在 generate() 函数外的 endpoint handler 内,不在 generate 闭包里),其余 30 端点都是 8 空格缩进。这与 Stage 1 改动模式完全一致,只是替换语句不同。已在 progress 三件套记录。
...
- 前端管 counter(0 / 1 / 2 / 3,达 3 禁用按钮)
- 后端只看 `context_prompt` 是否非空
- 删除 Stage 1 三档 UI 后,**也不要再传 `complexity` 字段**(后端会静默忽略,但传它无意义)
...
- ✅ 不动 tests/(交给 @tester,@tester 已同轮完成 sensor)
- ✅ py_compile 通过
...
## [2026-04-28] PM · 地毯审查发现 P0 阻塞 bug 已修 + 3 条 follow-up 入 PENDING

收到 Founder feedback "PM 审查必须毫无遗漏地毯式深挖" → 立即存项目 memory(`feedback_carpet_code_review.md`)+ 立刻补做地毯审查。

**P0 阻塞 bug(已修复 commit 709335c)**:
- stream.py + stream_en.py 31 端点用 `TEMPLATE.format(previous_output=ctx)`
- 用户上轮 prompt 含 `{` `}` 字符(`{role: ...}` / `{angle}` / JSON / Python f-string 等)时 → KeyError → SSE 端点 500 崩溃
- 修复:全 31 处改 `.replace("{previous_output}", ctx)`(任何字符都安全)
- 加防御 sensor:`assert "TEMPLATE.format(" not in stream_py` 防回归
- 测试 92 passed,push 到 sumai remote

**3 条 follow-up 入 PENDING**(非本轮修):
- F-1 P2: refinementRound 在 SSE 失败时卡死(应在成功回调 +1)
- F-2 P1: generateImageDescription GET URL 加 5000 字符 ctx 可能超 nginx 8KB header
- F-3 P2: ctx 后端截断时前端无提示
